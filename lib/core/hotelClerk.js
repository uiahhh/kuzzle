...
class HotelClerk {
  constructor (kuzzle) {
     ...
     *   {
     *      customers: Set([ 'connectionId' ]), // -> list of users subscribing
     *                                          //    to this room
     *      channels: {                         // -> room channels
     *        'roomId-<configurationHash>': {   // channel name
     *          scope: 'all|in|out|none',       // request scope filter,
     *                                          //   default: 'all'
     *          users: 'all|in|out|none'        // filter users notifications,
     *                                          //   default: 'none'
     *        }
     *      },
  ...
  /**
   * If the provided room ID does not exist anymore on the cluster, then delete
   * it and remove it from Koncorde
   *
   * @param {string} roomId
   */
  _removeRoomFromRealtimeEngine (roomId) {
    if (!this.kuzzle.cluster.realtime.hasRoom(roomId)) {

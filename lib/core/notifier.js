...
class NotifierController {
  ...
  async notifyDocument (rooms, request, scope, action, content) {
    ...
    await this.kuzzle.cluster.broadcast('cluster:notify:document', {
      action,
      content,
   ...
   * @return {Promise}
   */
  async notifyUser (room, request, scope, content) {
    await this.kuzzle.cluster.broadcast('cluster:notify:user', {
      content,
      request: request.serialize(),
      room,
      scope

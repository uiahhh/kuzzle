...
class StorageEngine {
   ...
   *
   * @returns {Promise.<boolean>} True if the cache has changed
   */
  async _add ({ index, collection, scope='public', notify=true } = {}) {
    let modified = false;
    ...
    if (notify && modified) {
      await this._kuzzle.cluster.broadcast('indexCache:add', {
        collection,
        index,
        scope,
      });
   ...
   */
  async _remove ({ index, collection, scope='public', notify=true } = {}) {
    let modified = false;
    ...
    if (notify && modified) {
      await this._kuzzle.cluster.broadcast('indexCache:remove', {
        collection,
        index,
        scope,
      });

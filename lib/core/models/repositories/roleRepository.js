...
class RoleRepository extends Repository {
  ...
  init () {
    ...
    this.kuzzle.onAsk('core:security:roles:reset', this._onReset.bind(this));
  }
  ...
  async validateAndSaveRole (role, options = {}) {
    ...
    await this.kuzzle.cluster.broadcast('role:update', { _id: role._id });
   ...
   * @returns Promise
   */
  async delete (role, options = {}) {
    if (['admin', 'default', 'anonymous'].indexOf(role._id) > -1) {
      throw errorsManager.get('security', 'role', 'cannot_delete');
    }

    const response = await this.kuzzle.repositories.profile.searchProfiles(
      [role._id],
      { from: 0, size: 1 });

    if (response.total > 0) {
      throw errorsManager.get('security', 'role', 'in_use', role._id);
    }

    const deleteResponse = await this.deleteFromDatabase(role._id, options);

...
class Node {
   ...
   */
  async discover () {
    debug('discover %s', this.sockets.pub.href);
    ...
    if (this.pool.size === 0) {
      debug('%s first node in the cluster - reset', this.sockets.pub.href);
  ...
  _removeNode (nodePub) {
    ...
    return this.kuzzle.cluster.cleanNode(node)

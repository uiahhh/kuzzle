...
class Node {
  ...
  broadcast (room, data) {
    if (!this.ready) {
      debug(
        '[%s][warning] could not broadcast "dump" action: node not connected to cluster',
        this.uuid);
  ...
  async sync (data) {
    ...
    switch (data.event) {
      ...
      case 'indexCache:remove':
        ...
        break;
      case 'profile':
        this.kuzzle.repositories.profile.profiles.delete(data.id);
        break;
      case 'role':
  ...
  async _onSubMessage (buffer) {
    ...
    switch (room) {
      ...
      case 'cluster:sync':
        ...
        break;
      case 'cluster:admin:dump':
        debugCluster('[sub][%s] %o', room, data);
        await this.kuzzle.janitor.dump(data.suffix);
        break;
      case 'cluster:admin:shutdown':
        debugCluster('[sub][%s] %o', room, data);
        process.kill(process.pid, 'SIGTERM');
        break;
      case 'cluster:admin:resetSecurity':
        debugCluster('[sub][%s] %o', room, data);
        this.kuzzle.repositories.profile.profiles.clear();
        this.kuzzle.repositories.role.roles.clear();
        break;
    }
  }

...
class StateManager {
  ...
  async sync (data) {
    ...
    const lockKey = `${index}/${collection}`;
    ...
    try {
      const state = await State.current(this.node.redis, index, collection);
      ...
      if (this.scheduledResync.has(lockKey)) {
        ...
        setImmediate(() => this.sync({collection, index, post: 'reset'}));
      }
    }
  }
   ...
   */
  async syncAll () {
    const tags = await this.node.redis.smembers('cluster:collections');

    const promises = [];

    for (const tag of tags) {
      const [index, collection] = tag.split('/');
      promises.push(this.sync({collection, index}));
    }

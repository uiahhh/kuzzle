...
class AdminController extends NativeController {
  ...
  async resetSecurity (request) {
    ...
    try {
      ...
      const { profileIds, roleIds } = await this.kuzzle.internalIndex.bootstrap
      ...
      await this.kuzzle.cluster.broadcast('cluster:admin:resetSecurity');
    }
    finally {
  ...
  async dump (request) {
    ...
    await this.kuzzle.cluster.broadcast('cluster:admin:dump', suffix);
  ...
  async shutdown (request) {
    ...
    await this.kuzzle.cluster.broadcast('cluster:admin:shutdown');
  ...
  async _waitForAction (request, promise, result) {

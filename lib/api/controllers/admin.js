...
class AdminController extends NativeController {
  ...
  async resetSecurity (request) {
    ...
    try {
      ...
      await this.kuzzle.cluster.broadcast('admin:resetSecurity');
    }
    finally {
  ...
  async dump (request) {
    ...
    await this.kuzzle.cluster.broadcast('admin:dump', suffix);
  ...
  async shutdown (request) {
    ...
    await this.kuzzle.cluster.broadcast('admin:shutdown');
